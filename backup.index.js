...
(async function () {
  ...
  async function fetchDiscogsUsername(token) {
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
        ...
        onload: (res) =>
          res.status === 200
            ? resolve(JSON.parse(res.responseText).username || null)
            : resolve(null),
  ...
  async function getCustomFields() {
    ...
    if (!token)
      return { error: true, status: 'No Token', responseText: 'Please enter and save your token.' };
    const username = await fetchDiscogsUsername(token);
    if (!username)
      return {
        error: true,
        status: 'Invalid Token',
        responseText: 'Could not fetch username. Please check your token.',
      };
    ...
    return new Promise((resolve) => {
      GM_xmlhttpRequest({
          ...
          if (response.status >= 200 && response.status < 300) {
            ...
            allCustomFields.forEach((f) => {
              if (f.type === 'dropdown' && f.options) {
                const opts = f.options.reduce((acc, opt) => {
                  acc[opt] = opt;
                  return acc;
                }, {});
        ...
        onerror: (error) =>
          resolve({ error: true, status: 'Network Error', responseText: JSON.stringify(error) }),
      });
    });
  }
  ...
  async function applyBulkEdit(selections) {
    ...
    if (itemsToUpdate.length === 0)
      return displayStatusMessage('No collection items found on this page to edit.', 'info');

    const fieldNameToId = allCustomFields.reduce((acc, field) => {
      acc[field.name.toLowerCase()] = field.id;
      return acc;
    }, {});
    const notesToUpdate = [];
    if (selections.mediaCondition)
      notesToUpdate.push({
        fieldId: fieldNameToId['media condition'],
        value: selections.mediaCondition,
      });
    if (selections.sleeveCondition)
      notesToUpdate.push({
        fieldId: fieldNameToId['sleeve condition'],
        value: selections.sleeveCondition,
      });
    if (selections.origin)
      notesToUpdate.push({ fieldId: fieldNameToId['origin'], value: selections.origin });
    if (selections.placement)
      notesToUpdate.push({ fieldId: fieldNameToId['placement'], value: selections.placement });
    ...
    for (let i = 0; i < itemsToUpdate.length; i++) {
      ...
      for (let j = 0; j < notesToUpdate.length; j++) {
        ...
        displayStatusMessage(
          `Updating item ${i + 1}/${itemsToUpdate.length} (field ${j + 1}/${
        ...
        await new Promise((resolve) => {
          GM_xmlhttpRequest({
              ...
              if (response.status === 200) {
                try {
                  ...
                  if (responseData.errors && responseData.errors.length > 0) {
                    displayStatusMessage(
                      `GraphQL Error on item ${collectionItemId}: ${responseData.errors[0].message}`,
                      'error',
                      0,
                    );
                    console.error(
                      `DHP: GraphQL error on item ${collectionItemId}:`,
                      responseData.errors,
                    );
                  } else {
                  ...
                  displayStatusMessage(
                    `Error parsing response for item ${collectionItemId}.`,
                    'error',
                    0,
                  );
                ...
                displayStatusMessage(
                  `Error: ${response.status} for item ${collectionItemId}. See console.`,
                  'error',
                  0,
                );
                console.error(
                  `DHP: API error for item ${collectionItemId}: Status ${response.status}`,
                  response,
                );
              }
            ...
            onerror: (error) => {
              displayStatusMessage(
                `Network error for item ${collectionItemId}. See console.`,
                'error',
                0,
              );
    ...
    displayStatusMessage(
      `Finished: Successfully updated ${successfulFieldsUpdated} fields across ${itemsToUpdate.length} items. Reloading...`,
      'success',
      0,
    );
    setTimeout(() => location.reload(), 1500);
  ...
  Object.assign(panelContainer.style, {
    ...
    color: 'white',
    transform: GM_getValue(PANEL_STATE_STORAGE_KEY, true)
      ? 'translateX(0)'
      : 'translateX(calc(100% - 30px))',
  });
  document.head.appendChild(
    document.createElement('style'),
  ).innerHTML = `.hidden { display: none !important; }`;
  ...
  if (fetchedCustomFields && !fetchedCustomFields.error) {
    ...
    if (isCollectionPage) {
      ...
      Object.assign(bulkEditTitle.style, {
        margin: '0 0 10px 0',
        fontSize: '14px',
        color: 'white',
        textAlign: 'center',
      ...
      contentArea.appendChild(
        createSelectField('Media Condition', 'dhp-media', fetchedCustomFields.mc || {}),
      );
      contentArea.appendChild(
        createSelectField('Sleeve Condition', 'dhp-sleeve', fetchedCustomFields.sc || {}),
      );
      contentArea.appendChild(
        createSelectField('Origin', 'dhp-origin', fetchedCustomFields.or || {}),
      );
      contentArea.appendChild(
        createSelectField('Placement', 'dhp-placement', fetchedCustomFields.pl || {}),
      );
    ...
    if (window.location.pathname.startsWith('/artist')) {
      ...
      Object.assign(inCollectionBtn.style, {
        padding: '10px',
      ...
      inCollectionBtn.onmouseover = () =>
        (inCollectionBtn.style.backgroundColor = 'rgb(20, 90, 180)');
      inCollectionBtn.onmouseout = () =>
        (inCollectionBtn.style.backgroundColor = 'rgb(25, 118, 210)');
      inCollectionBtn.addEventListener('click', () =>
        Array.from(
          document.querySelectorAll(`tr:not(:has([class*=collection_], [class*=wantlist_]))`),
        ).map((e) => e.parentNode && e.parentNode.removeChild(e)),
      );
    ...
    if (window.location.pathname.startsWith('/mywantlist')) {
      ...
      Object.assign(uniqueItemsBtn.style, {
        padding: '10px',
      ...
      uniqueItemsBtn.onmouseover = () =>
        (uniqueItemsBtn.style.backgroundColor = 'rgb(220, 130, 0)');
      uniqueItemsBtn.onmouseout = () => (uniqueItemsBtn.style.backgroundColor = 'rgb(255, 159, 0)');
      uniqueItemsBtn.addEventListener('click', () => {
        const find = (selector, root = window.document) =>
          Array.from(root.querySelectorAll(selector));
        find('tr[class*=wantlist]')
          .map((release) => ({
            artist: find('td.artist_title a', release)[0]?.innerText || '',
            title: find('td.artist_title a', release)[1]?.innerText || '',
            rel: release,
          }))
          .filter((release, index, list) =>
            list.some(
              (_release, _index) =>
                release.title.toLowerCase() === _release.title.toLowerCase() &&
                release.artist.toLowerCase() === _release.artist.toLowerCase() &&
                index < _index,
            ),
          )
    ...
    titleMsg.innerText = 'Authentication Failed';
    titleMsg.fontWeight = 'bold';
    ...
    if (fetchedCustomFields.responseText) {
      ...
      responseTitle.innerText = 'API Response:';
      responseTitle.style.marginTop = '5px';
      info.appendChild(responseTitle);
      const responseBody = document.createElement('pre');
      try {
        responseBody.innerText = JSON.stringify(
          JSON.parse(fetchedCustomFields.responseText),
          null,
          2,
        );
      } catch (e) {
        responseBody.innerText = fetchedCustomFields.responseText;
      }
      Object.assign(responseBody.style, {
