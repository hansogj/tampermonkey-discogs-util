...
export function displayStatusMessage(text: string, type: MessageType = 'error', duration: number = 5000): void {
...
export function createSelectField(labelText: string, id: string, optionsMap: Record<string, string>): HTMLElement {
    const container = document.createElement('div');
...
export const find = (selector: string, root: ParentNode = document): HTMLElement[] => Array.from(root.querySelectorAll(selector)) as HTMLElement[];
