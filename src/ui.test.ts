...
const mockGmValueStore: Record<string, unknown> = {};
global.GM_getValue = vi.fn(
  (key: string, defaultValue: unknown) => mockGmValueStore[key] ?? defaultValue,
);
global.GM_setValue = vi.fn((key: string, value: unknown) => {
  mockGmValueStore[key] = value;
});
global.GM_deleteValue = vi.fn((key: string) => {
  delete mockGmValueStore[key];
});
