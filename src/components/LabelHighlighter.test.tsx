...
const MockMutationObserver = vi.fn(function (this: unknown, callback: MutationCallback) {
...
describe('LabelHighlighter', () => {
  ...
  const createMockLabelLink = (labelName: string, href = '/label/Some-Label'): HTMLElement => {
  ...
  it('should use DEFAULT_HIGHLIGHTED_LABELS if no custom labels are stored', () => {
    ...
    expect(mockLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.poor));
  ...
  it('should use custom highlighted labels if they are stored', () => {
    ...
    expect(mockLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.good));
  ...
  it('should correctly apply styles for "fair" quality', () => {
    ...
    expect(mockLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.fair));
  ...
  it('should correctly apply styles for "veryGood" quality', () => {
    ...
    expect(mockLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.veryGood));
    expect(mockLabelLink.style.color).toBe('white');
  });
  ...
  it('should re-highlight labels when MutationObserver triggers', async () => {
    // Keep async for act
    // Prepare initial DOM
    const initialLabelLink = createMockLabelLink(DEFAULT_HIGHLIGHTED_LABELS.good[0]);
    ...
    const observerInstance = (MockMutationObserver as vi.Mock).mock.instances[0];
    ...
    expect(newLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.poor));
    expect(newLabelLink.style.color).toBe('white');

    // Initial label should still be styled
    expect(initialLabelLink.style.backgroundColor).toBe(hexToRgb(LABEL_QUALITY_COLORS.good));
  });
