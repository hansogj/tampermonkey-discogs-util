...
export function CollectionPanel() {
  ...
  const { mutate: performBulkEdit, isPending: isBulkEditing } = useMutation<
    string,
    Error,
    ApplyEditSelections
  >({
  ...
  if (error) {
    return (
      <div style={errorStyle}>
        ...
        <pre style={errorPreStyle}>{error.message}</pre>
      </div>
    );
  }
  ...
  return (
    <div id="bulk-edit-section">
      ...
      <button
        ...
        onMouseEnter={(e) => {
          if (!isBulkEditing) e.currentTarget.style.backgroundColor = '#00a300';
        }}
        onMouseLeave={(e) => {
          if (!isBulkEditing) e.currentTarget.style.backgroundColor = '#00c700';
        }}
      >
