...
global.GM_getValue = vi.fn((key: string, defaultValue: any) => {
...
let queuedXhrResponses: { response: { status: number, responseText: string }, type: 'onload' | 'onerror' }[] = [];

// Mock GM_xmlhttpRequest implementation
global.GM_xmlhttpRequest = vi.fn(async (options: Tampermonkey.Request) => { // Make it async
    ...
    if (nextResponse) {
        ...
        if (nextResponse.type === 'onload') {
            options.onload?.({ status: nextResponse.response.status, responseText: nextResponse.response.responseText } as Tampermonkey.Response<any>);
        } else {
            options.onerror?.({ error: nextResponse.response.responseText } as Tampermonkey.ErrorResponse);
        }
    } else {
...
describe('api.ts', () => {
    ...
    const queueXmlHttpRequest = (response: { status: number, responseText: string }, type: 'onload' | 'onerror' = 'onload') => {
    ...
    describe('getCustomFields', () => {
        ...
        it('should throw an error if username cannot be fetched', async () => {
            ...
            await expect(getCustomFields()).rejects.toThrow('Could not fetch username. Please check your token.');
            expect(GM_xmlhttpRequest).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('applyBulkEdit', () => {
        ...
        const mockCustomFields: CustomField[] = [
            { id: mockField1Id, name: 'Media Condition', type: 'dropdown', options: ['Mint', 'Near Mint'] },
            { id: mockField2Id, name: 'Sleeve Condition', type: 'dropdown', options: ['Mint', 'Near Mint'] },
        ...
        it('should throw an error if no collection items found', async () => {
            await expect(applyBulkEdit({})).rejects.toThrow('No collection items found on this page to edit.');
            expect(GM_xmlhttpRequest).not.toHaveBeenCalled();
        });
        ...
        it('should apply bulk edit successfully to multiple items', async () => {
            ...
            expect(GM_xmlhttpRequest).toHaveBeenCalledWith(expect.objectContaining({
                ...
                data: JSON.stringify({
                    ...
                    "variables": { "input": { "discogsItemId": 101, "discogsNoteTypeId": mockField1Id, "noteText": "Mint" }},
        ...
        it('should correctly increment successfulFieldsUpdated only for non-error GraphQL responses', async () => {
            ...
            queueXmlHttpRequest({ status: 200, responseText: JSON.stringify({ errors: [{ message: 'GraphQL error' }] }) });
