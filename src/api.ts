...
export async function applyBulkEdit(selections: ApplyEditSelections): Promise<string> {
    const itemsToUpdate = Array.from(document.querySelectorAll('div.MuiDataGrid-row[data-id]')) as HTMLElement[];
    ...
    for (const item of itemsToUpdate) {
        ...
        for (const note of notesToUpdate) {
            const graphqlPayload = {
                ...
                "variables": { "input": { "discogsItemId": collectionItemId, "discogsNoteTypeId": note.fieldId, "noteText": note.value }},
